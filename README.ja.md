...
VPresentation のソースコードは [MIT ライセンス](LICENSE) です。

**これは THA4 の学習済みモデル重みには適用されません。** 重みは THA4 作者から
別途 **CC BY-NC 4.0（非商用）** で配布されています（[`tools/fetch_weights.sh`](tools/fetch_weights.sh) 参照）。
teacher を直接使う場合でも、そこから蒸留した per-character モデル
（[`DISTILL.md`](DISTILL.md) 参照）を使う場合でも、この重みに依存する利用は
本リポジトリの MIT ライセンスとは別に、その非商用制限に従う必要があります。
商用利用には THA4 作者との別途の取り決めが必要です。
